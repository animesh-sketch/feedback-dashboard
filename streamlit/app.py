...
<table style="width:100%;border-collapse:collapse;font-size:13px;">'''
                    ...
                    # ── Key Callouts for footer ───────────────────────────────
                    _fc_score_clr = "#059669" if (_avg_i or 0) >= 80 else "#d97706" if (_avg_i or 0) >= 60 else "#dc2626"
                    _fc_pr_clr    = "#059669" if pass_rate_i >= 80 else "#d97706" if pass_rate_i >= 60 else "#dc2626"
                    _footer_callouts = (
                        f'<div style="margin:16px 0;padding:16px;background:#f8fafc;border-radius:10px;border:1px solid #E2EAF6;">'
                        f'<div style="font-size:10px;font-weight:800;letter-spacing:0.14em;text-transform:uppercase;color:#64748b;margin-bottom:10px;">📌 Key Callouts</div>'
                        f'<div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:10px;">'
                        f'<div style="background:#fff;border:1px solid {_fc_score_clr}33;border-left:3px solid {_fc_score_clr};border-radius:8px;padding:8px 14px;flex:1;min-width:120px;">'
                        f'<div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:0.1em;">Avg Score</div>'
                        f'<div style="font-size:20px;font-weight:900;color:{_fc_score_clr};">{_avg_i or "—"}%</div></div>'
                        f'<div style="background:#fff;border:1px solid {_fc_pr_clr}33;border-left:3px solid {_fc_pr_clr};border-radius:8px;padding:8px 14px;flex:1;min-width:120px;">'
                        f'<div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:0.1em;">Pass Rate</div>'
                        f'<div style="font-size:20px;font-weight:900;color:{_fc_pr_clr};">{pass_rate_i}%</div></div>'
                        f'<div style="background:#fff;border:1px solid #E2EAF6;border-left:3px solid #2563EB;border-radius:8px;padding:8px 14px;flex:1;min-width:120px;">'
                        f'<div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:0.1em;">Total Audits</div>'
                        f'<div style="font-size:20px;font-weight:900;color:#0B1F3A;">{total_i:,}</div></div>'
                        f'</div>'
                    )
                    # Top critical/warning insight callout
                    _top_ins = next((i for i in _sorted_ins if i.get("type") in ("critical","warning")), None)
                    if _top_ins:
                        _ti_clr = "#dc2626" if _top_ins["type"] == "critical" else "#d97706"
                        _footer_callouts += (
                            f'<div style="background:#fff;border:1px solid {_ti_clr}33;border-left:3px solid {_ti_clr};border-radius:8px;padding:8px 14px;margin-bottom:8px;">'
                            f'<div style="font-size:11px;font-weight:700;color:{_ti_clr};margin-bottom:3px;">{_top_ins["title"]}</div>'
                            f'<div style="font-size:11px;color:#475569;line-height:1.4;">{_top_ins["detail"]}</div>'
                            f'</div>'
                        )
                    # Bottom parameter callout
                    _fp_worst = sorted([p for p in _param_avgs if p["pct"] < 80], key=lambda x: x["pct"])
                    if _fp_worst:
                        _fpw = _fp_worst[0]
                        _footer_callouts += (
                            f'<div style="background:#fff;border:1px solid #fecdd3;border-left:3px solid #dc2626;border-radius:8px;padding:8px 14px;">'
                            f'<div style="font-size:9px;font-weight:800;color:#dc2626;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:3px;">Lowest Parameter</div>'
                            f'<div style="font-size:11px;font-weight:700;color:#0B1F3A;">{_fpw["col"]} — {_fpw["pct"]}% (gap to 80%: −{round(80-_fpw["pct"],1)}%)</div>'
                            f'</div>'
                        )
                    _footer_callouts += '</div>'

                    _footer = (
                        _footer_callouts +
                        f'<div style="border-top:1px solid #E2EAF6;margin-top:16px;padding-top:16px;text-align:center;">'
